...
def download_csv():
    ...
    if os.environ.get("POSTGRES_URL", None) is not None:
        if "og" in cluster_type:
            ...
            keys = ("area_km2", "grid_dist_km", "ST_AsGeoJSON(centroid) as lnglat")
...
def fetch_centroids():
    ...
    if "og" in cluster_type:
        ...
        records = query_filtered_og_clusters(state, STATE_CODES_DICT, keys=keys,)
    else:
        keys = (
            "adm1_pcode",
            "cluster_all_id",
        ...
        records = query_filtered_clusters(state, STATE_CODES_DICT, keys=keys,)
...
def random_clusters():
    ...
    feature = dict(
        lng=lnglat["coordinates"][0], lat=lnglat["coordinates"][1], properties=resp
    )
    ...
    if os.environ.get("POSTGRES_URL", None) is not None:
        if "og" in cluster_type:
            ...
            records = query_filtered_og_clusters(
                ...
                distance_grid=[
                    request.form.get("ogmindtg"),
                    request.form.get("ogmaxdtg"),
                ],
